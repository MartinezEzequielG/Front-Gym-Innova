    ...
    >
      <Stack spacing={1.25}>
        ...
        <Stack direction="row" alignItems="center" justifyContent="space-between" gap={1}>
          <Chip
            size="small"
            label={client.status ? client.status.toUpperCase() : '—'}
            sx={{
              borderRadius: 999,
              fontWeight: 900,
              backgroundColor: client.status === 'activo' ? alpha('#10b981', 0.12) : alpha('#111827', 0.06),
              color: client.status === 'activo' ? '#0f766e' : alpha('#111827', 0.85),
            }}
          />
      ...
      <Box sx={{ p: { xs: 2, md: 4 }, maxWidth: 1250, mx: 'auto' }}>
        ...
        <Box sx={{ display: { xs: 'block', md: 'none' } }}>
          <Stack spacing={1.5}>
            ...
            {filtered.map((c) => (
              <ClientCard key={c.id} client={c} onDelete={requestDelete} deleting={deleting} onEdit={handleOpenEdit} />
            ))}
          </Stack>
        </Box>
